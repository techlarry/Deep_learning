...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        running_var = momentum * running_var + (1 - momentum) * sample_var
        # Store cache
        cache = (eps, sample_mean, sample_var, x, x_hat, gamma)
    #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    eps, sample_mean, sample_var, x, x_hat, gamma = cache
    N = x.shape[0]
    dx_hat = dout*gamma
    dvar = np.sum(dx_hat*(x-sample_mean)*(-0.5)*np.power(sample_var + eps, -1.5), axis=0, keepdims=True)
    dmean = np.sum(-dx_hat/np.sqrt(sample_var+eps), axis=0, keepdims=True) +\
                dvar*np.mean(-2*(x-sample_mean), axis=0, keepdims=True)
    dx = dx_hat/np.sqrt(sample_var+eps) + dvar*2.*(x-sample_mean)/N + 1./N*dmean
...
def batchnorm_backward_alt(dout, cache):
    ...
    """
    bn_param, dgamma, dbeta = None, None, None
    ###########################################################################
    # TODO: Implement the backward pass for batch normalization. Store the    #
    # results in the dx, dgamma, and dbeta variables.                         #
    #                                                                         #
    ...
    dmean = np.sum(-dx_hat/np.sqrt(sample_var+eps), axis=0, keepdims=True) +\
                dvar*np.mean(-2*(x-sample_mean), axis=0, keepdims=True)
    dx = dx_hat/np.sqrt(sample_var+eps) + dvar*2.0*(x-sample_mean)/N + 1.0/N*dmean
    dgamma = np.sum(dout*x_hat, axis=0, keepdims=True)
    dbeta = np.sum(dout, axis=0, keepdims=True)
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(H_out):
        for j in range(W_out):
            out[:, :, i, j] = np.max(x[:, :, i*S:i*S+HH, j*S:j*S+WW], axis=(2,3))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in xrange(H_out):
        for j in xrange(W_out):
            area = x[:,:,i*S:i*S+HH, j*S:j*S+WW]
            area_max = np.max(area, axis=(2,3))
            mask = (area == (area_max)[:,:,None,None])
            dx[:,:,i*S : i*S+HH, j*S : j*S+WW] += mask * (dout[:,:,i,j][:,:,None,None])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
